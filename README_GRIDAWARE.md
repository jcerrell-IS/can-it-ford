...
|---|---|---|---|---|
| `compact_sedan` | Toyota Yaris (2010, NCAC/CCSA FE model) | 1100 kg | 4.30 x 1.70 x 1.47 | uniform-box fallback, **not** a measured tensor (the NHTSA database ends Nov 1998 and holds no Yaris; a measured tensor does exist at the DOI in this cell, slide 7, and is deliberately not wired in); mass/bbox from [crash-validated FE model](https://doi.org/10.13021/G8JS5D) |
...
| File | Description |
|---|---|
| `data/all_runs_inventory.csv` | **Primary source for the paper's coupled sweep.** 17 runs in `render_s2` on the watertight Yaris hull. These runs are not "gated" in any recorded sense: this file carries the gate inputs (`passthrough_max_frac`, `C3_oob_particle_frames`, `leaked_particle_frames`, `determinism_identical`) and has no pass/fail column, and no artifact in this repo records a gate outcome. 7 of the 17 exceed a 10 percent particle-passthrough threshold and are flagged, not excluded. `n_vehicle` and `fill_ratio` are set by `n_grid`, not by mass: 3846 / 1.026243 at g48 (3 runs), 8905 / 1.002440 at g64 (11 runs), 29804 / 0.994089 at g96 (3 runs). **Not currently committed:** the `data/*` ignore rule excludes it, so a fresh clone will not contain this file |
