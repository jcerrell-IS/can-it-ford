...
- Velocity (parameter 00055) confirmed unavailable at this gage. Checked directly against both the continuous time series and 731 field-measurement records going back years: zero velocity readings exist in either, even for measurements taken with methods that inherently compute velocity as an intermediate step, such as mid-section current-meter surveys and Acoustic Doppler Current Profiler readings. The modernized USGS Water Data API does not surface that intermediate value for this site. Sweep's velocity bound (0 to 3.0 m/s) remains ungrounded against USGS data for this gage.

  UPDATE: see citations/smith_modra_felder_2019_velocity_grounding.md for the follow-up on this. Short version, that paper does NOT hand over a simple tested velocity range either, its full-scale prototype tests were static, stagnant water only, and the velocity in its stability curves is a calculated quantity, not directly measured. The usable output from that paper is an invertible equation (Eq. 6), not a plain number. Read that file before writing any velocity-grounding sentence in the paper.
