...
"SLIDE within 4 frames" is a measurement, not an independent confirmation:
onset at frames 3, 3 and 4 against `DRIFT_THRESHOLD = 0.05` m.
`data/failure_modes_by_run_classified.csv` and `data/failure_modes_by_run.json`
(keyed by run id, covering all 17 gated runs via
`simulation/failure_modes.py, classify_timeseries()`, regenerated by
`analysis/classify_failure_modes.py`) classify all three as SLIDE, consistent
with 16 of 17 runs overall (the exception is `sweepV_g64_v0p5`, classified
STUCK).

This does NOT corroborate the table above. `simulation/failure_modes.py` reads
the same `_incoming/<run>/metrics.csv` that the table was built from, so it
restates one rollout under an explicit criterion rather than confirming it from
a second source. The classification also inherits SSF 1.42
...
Both stores replace `failure_modes_result.json`, which was keyed by AR&R class,
