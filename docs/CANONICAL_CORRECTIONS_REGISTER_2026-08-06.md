...
**G25. THE NINE MULTI-REPORT UNCITED PAPERS, TRIAGED AGAINST THE FREE-RIGID FORCE DEFICIT. T1 for the catalogue arithmetic and the DOI resolutions, T2 for each paper's content.**

**NUMBERING, 2026-08-21 branch reconciliation: this was G17 on `claude/add-ci-checks`, and collided with the pre-existing G17 (P-2 commensurability, line 734) on merge with `claude/r8-register`. Renumbered to 25/25a, the sole existing citer of this branch's `G17` (its own falsifier line) repointed in the same pass.**
...
**G25a. TWO DEFECTS IN THE CATALOGUE ITSELF, found while verifying it. T1.**
...
**FALSIFIER:** G25 dies if the catalogue is re-parsed and yields other than 205 data rows,
...
a. R10 agent `a2bcb1f09`, read-directly.
b. R10 agent `a25a0c14c`, read-directly, naming the address: **register E1's own document,
   DOI `10.13021/G8JS5D`**, which this project already cites as its hull provenance.
c. The `Simulation Ready Vehicle Mesh Assets` deep search, which records the CCSA/NCAC
...
(Relettered from a numbered list 2026-08-21: three sub-points under Section J's own
top-level 1/2/3 numbering convention were colliding with it after this content merged in
from `claude/r8-register`. Content unchanged.)
