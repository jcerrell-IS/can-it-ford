...
| A6 | **Four bibliography DOIs verified and supplied**, plus one year error (`ccsa2010yaris`, bib 2010 vs DataCite 2016); year audit found it **isolated**, 8 of 9. **CORRECTED TWICE 2026-08-19: the IEEE bib entry count is BRANCH-DEPENDENT** - 21 on `claude/r5-research`, 36 on `claude/add-ci-checks` after `ffc05d9`. My first correction blamed a regex; unit 62 showed both regexes agree and it is a branch difference. **Never quote a bib count without its checkout.** **And the live paper compiles from `deliverables/paper/overleaf/refs.bib` (11 entries, 0 VERIFY), where `ccsa2010yaris`, `alqadami2022` and `martinezgomariz2018` are ABSENT** | `BIB_MOVED_2026-08-19` |
