...
| A6 | **Four bibliography DOIs verified and supplied**, plus one year error (`ccsa2010yaris`, bib 2010 vs DataCite 2016); year audit found it **isolated**, 8 of 9. **CORRECTED 2026-08-19: the IEEE bib has 36 entries, not the 21 I reported** (my regex matched only lowercase `@type` at line start). **And the live paper compiles from `deliverables/paper/overleaf/refs.bib` (11 entries, 0 VERIFY), where `ccsa2010yaris`, `alqadami2022` and `martinezgomariz2018` are ABSENT** | `BIB_MOVED_2026-08-19` |
