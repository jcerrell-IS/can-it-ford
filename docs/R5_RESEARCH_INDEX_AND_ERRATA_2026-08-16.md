...
| `R5_RESEARCH_FORCE_CONVERGENCE_2026-08-19.md` | **RETRACTED same day, 7 blocking.** The arithmetic reproduces exactly; the quantity is not a force. Read as a worked failure |
| `R5_RESEARCH_CLAUDEMD_STALE_SOUNDSPEED_2026-08-19.md` | **for the CLAUDE.md maintainer**: `:524` says the sound speed was "never swept here"; jobs 895330/895378 swept it across 3 decades on 2026-08-07 |
...
| `R5_RESEARCH_VALIDATION_PROTOCOL_UNUSED_2026-08-19.md` | **strongest form of the citation finding**: a paper-ready validation protocol was commissioned FOR this project (81 papers, 2026-07-15) and none of its vocabulary reached the paper |
...
| 34 | E8's operative rule quoted as ending "...without written permission." | unit 31 / `GNN_AND_MESH_LICENCE:22` | **TRUNCATED, and it hid a second route.** The register reads "without written permission **or a confirmed licence**". `or a confirmed licence` appeared in **zero** of my 32 documents. **FOR D2: two routes clear E8, not one.** Found by a systematic audit of all 82 block quotes; **third truncation of this dispatch** after units 36 and 40 | unit 43 |
| 35 | `corpus_cited_status`, THE NOVELTY GUARD, reports the four fording papers as `cited` | the tool's own verdict | **"cited" means repo-wide TEXT PRESENCE, not paper citation.** All four (He 2026, Wasfy 2015, Khapane 2014, Pazouki 2016) return `cited` and resolve to four different research notes in `docs/`; **all four have ZERO `.tex` occurrences**, and the one `.bib` hit is an Elicit export with a hash key. **Same conflation as erratum 4.** Latent, not live: the paper makes no first-of-kind claim today | unit 45 |
| 36 | unit 45's `.tex` absence checked against local `paper/` only | unit 45 §5.2 | **PURSUED and bounded.** Confirmed zero across FOUR `.tex` states (local x2, `overleaf/main`@`6466dfa`, `92ce4de`), but a **positive control disqualifies all of them as current**: `warpmpm` returns 0 on `92ce4de` though the relabelling is known landed. **The live Overleaf head is unreachable** (connector auth fails; refs 18 days stale). Absence is proven for stale states only. **New FLAG-6** | unit 46 |
| 37 | my E8 documents (units 31, 37) frame E8 as an OPEN blocker | units 31, 37 | **E8 is CLEARED**, D2 `a386704`, permission granted, global, permits republication. **My licence-absence finding is NOT superseded**: "no licence exists to confirm" is why permission was the necessary route, so it is evidence the clearance rests on. **The FHWA acknowledgement SURVIVES and is still unmet** (`FHWA` in 0 `.tex`, control `CCSA` in 3, re-measured 2026-08-18). Redistribution rights and acknowledgement are different obligations | unit 48 |
| 38 | "four uncited MPM method papers" (unit 47) | unit 47 §2 | **UNDERSTATED by an order of magnitude.** Reading the full 205-row TSV rather than the README's highlights gives **37** uncited MPM-method papers, 7 of them multi-report, including **GIMP (2004)** and volumetric locking. And the bibliography contains **zero** MPM method citations of any kind: **Sulsky is absent by name, title, DOI, DOI-stem and collaborator**, with positive controls firing. Their 138/67 headline reproduces exactly from their own data | unit 49 |
| 39 | **WHAT_SURVIVES A2**: "the bibliography has 21 entries; only 3 are actually `\cite`d" | my own consolidation, unit 41 | **WRONG, AND I INTRODUCED IT.** True: **11** distinct keys in `paper/conference_101719.tex`, **14** in the canonical file, **14 rendered references and 51 inline citations** in the compiled PDF (2026-08-04). Erratum 4 is correct but scoped to the **8 catalogued DOIs**; **consolidating it, I dropped the qualifier**, turning a true scoped claim into a false general one. Same failure mode as the three truncated quotations, in the one document meant to prevent exactly this | unit 50 |
| 40 | **WHAT_SURVIVES A1**: "10/41 report a depth-velocity threshold" | units 3, 12 | **9/41**, not 10. Two independent tests agree and the 9 rows are enumerated (2, 5, 7, 16, 26, 35, 37, 38, 39). **The friction 9/41 is CONFIRMED** and I nearly broke it: a regex caught "units not specified" in row 40 and binned a substantive "friction coefficient = up to 1.89". **Second false row found in the entry point**, which is why the audit was worth running | unit 51 |
| 41 | **WHAT_SURVIVES A3**: catalog recall "measured three independent ways" | my own consolidation, unit 41 | **TWO, not three.** `AUTHOR_SWEEP` §4 states it plainly: 16/32 loose at 1 hop and 7/14 strict at 2 hops. I counted `7/15` and `7/14` as separate; they are the **same seven papers** with a borderline case excluded (erratum 10). **Broke this project's own "one source cited twice is not two sources" rule, in the row calling itself the most robust finding.** THIRD false row of twelve; audit complete, A4-A6 and A10-A12 hold | unit 52 |
| 42 | unit 56: the project declined to report a GCI bound on an unconverged study, when the protocol asks for one | unit 56 first draft | **WITHDRAWN, AND THE PROJECT IS RIGHT.** No GCI is computable: the grids 48/64/96 give **r21=1.500000 vs r32=1.333333**, a non-constant refinement ratio, so no apparent order `p` exists at all. `physics_gates_literature.py:60-64` encodes it and `params_check.py` returns "cannot compute apparent order p" for all three masses. And it was **not silence**: `limitations.tex:13` already says "No Grid Convergence Index is computable ... GCI requires monotone refinement behavior". **I criticised correct work for an omission that cannot be done** | unit 58, physics-skeptic |
| 43 | unit 56: "nothing from the protocol reached the paper"; "the paper never uses the word convergence"; "built to be usable with what the project already had" | unit 56 first draft | **THREE WITHDRAWN.** 3 of the 81 catalog DOIs ARE in the paper bib, two under the header `% NUMERICS AND UNCERTAINTY: methods used or recommended by this work`; `not grid-converged` appears **3 times** in the PDF, each refusing a magnitude; and the 36-run sweep is `data/track1_sweep_v2/`, which **CLAUDE.md marks DEPRECATED**. Survives narrowly: none is `\cite`d and none is in the compiled reference list | unit 58 |
| 44 | unit 56 measured "the paper" | unit 56 first draft | **SCOPE ERROR, and it is the root cause of erratum 42 and 43.** I measured ONE artifact, the 7-page compiled PDF, and generalised to "the work". A second, longer paper exists at `deliverables/paper/overleaf/` whose `limitations.tex`, `results.tex` and `future_work.tex` address exactly what I called absent. **I never looked** | unit 58 |
| 45 | "the bibliography has 21 entries" -> unit 61's claim that this was a too-strict-regex bug giving 36 | unit 61 | **RETRACTED SAME DAY by unit 62.** Both regexes agree on both copies; it is a **BRANCH difference**: worktree `claude/r5-research` 9,503 B / 21 entries, main `claude/add-ci-checks` 16,906 B / 36. `ffc05d9` added 144 lines on a branch mine does not contain. **21 was correct for this branch; there was no regex bug and no "fourth regex failure".** Real lesson: a bare bib count is branch-dependent, so name the checkout | unit 62 |
| 46 | my bibliography findings target `paper/can_it_ford_references_IEEE.bib` | units 3, 7, 32, FLAG-4 | **The live paper compiles from `deliverables/paper/overleaf/refs.bib`**, 11 entries, **0 VERIFY markers**. Only 6 keys are shared. **`ccsa2010yaris`, `alqadami2022` and `martinezgomariz2018` are ABSENT from the live bib**, so three findings (incl. FLAG-4) concern a file the current paper does not use. Not wrong, but differently prioritised than I implied | unit 61 |
| 47 | unit 45: the four fording papers have zero `.bib` presence | unit 45 | **STANDS AS MEASURED, and the world then moved.** They are in the IEEE bib now via `ffc05d9` (2026-08-18 00:19, branch `claude/add-ci-checks`), ~80 min AFTER I measured; `ffc05d9^` had the He DOI at **0**. **Not a false zero.** I cannot show causation, only sequence. All four remain `\cite`d in **0** `.tex`, so the citation gap is still open | unit 61 |
| 48 | WHAT_SURVIVES A6, amended by unit 61 to say the bib has 36 entries | unit 61 | **A6's 36 is branch-specific.** Corrected to name both: 21 on `claude/r5-research`, 36 on `claude/add-ci-checks`. The four verified DOIs and the `ccsa2010yaris` year error are unaffected, both having been checked per entry rather than by the total | unit 62 |
| 49 | unit 64: our 0.55 friction is "nearly double the flood literature's convention", the chain is new, nobody has run a mu control, and the verdicts are conservative | unit 64 | **RETRACTED IN FULL, 5 blocking.** (a) **The whole chain is register G4a, dated 2026-08-07, citing the SAME report `65474f37`.** (b) **G4b warns verbatim against my exact framing**: "anyone reading only the convention half will try to resurrect 0.30 as best-sourced and will be wrong"; G4 refutes 0.30 as a wet-road value with measurements at **0.52-0.68**, so 0.55 is INSIDE the band. (c) **The physics is backwards**: `sweepV_g64_v0p5` misses SLIDE by **0.013732 m** because friction decelerates it below the speed gate, so lower mu pushes it TOWARD slide, making 0.55 **anti-conservative**. (d) `data/mu_sweep_results.csv` exists. (e) `conference_101719_1.tex:205` already names the mu=0.30 rerun as "the first thing to check". **Root cause: I asserted a parameter without reading the register, the rule I had quoted to siblings** | unit 65, physics-skeptic |
| 50 | unit 64's minor citation values | unit 64 | Toda 2013 is **0.26/0.57**, not 0.6 (register `:259`); Smith 2019 is **~0.78**, not 0.76; "690214: 0 files" is wrong (**4 numeric-coincidence hits**, substantively absent); my repo counts (azhar 86, Wong 13) **do not reproduce** and are withdrawn | unit 65 |
| 51 | unit 71: the surge force does not converge, on 4 grids with N=5 error bars | unit 71 | **RETRACTED, 7 blocking, 6 structural.** (a) **`M*dv_cm/dt` is not a force**: `v_cm` is OVERWRITTEN each substep by a PIC reprojection of a mixed water+rigid grid field (`mpm_utils.py:1434`), no force accumulator; register **D6f** already brands this accessor "numerical, not physical". (b) **peak_all is 0.92-1.42x the vehicle's own WEIGHT** horizontally, 36-58x a Cd=1 drag anchor. (c) **substeps 8/11/16/21**, a 2.6x rise in dissipative reprojections monotone with refinement, untested as the cause. (d) **The settle claim is INVERTED**: `stationarity.py` says discard 25-67 of 91 frames, >8 in 20/20, and my peak_ex at frames 3-7 is inside the discard region in 20/20. (e) **The "N=5" repeats share `seed=0`**, no `--seed` flag exists, so the spread is GPU round-off, not scatter. (f) `integral|F|dt` is total variation, not impulse. (g) Verdicts flip under a centred stencil and under 2 of 7 estimators | unit 72, physics-skeptic |
