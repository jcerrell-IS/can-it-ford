...
| 45 | "the paper's bibliography has 21 entries" | units 3, 32, A6 | **36 entries.** My `grep -oE "^@[a-z]+\{"` was anchored to line start and matched only **lowercase** entry types, so every `@Article{`/`@InProceedings{` was invisible. Same failure mode as unit 51's friction regex and unit 53's em-dash test | unit 61 |
