...
`failure_modes_by_run.json` covers all 17 gated runs, keyed by run id: 16 SLIDE
and 1 STUCK (`sweepV_g64_v0p5`, whose 0.0568 m peak surge drift clears the
0.05 m threshold in magnitude but never holds the joint drift-and-speed
condition for the required 3 consecutive frames). It replaces
`failure_modes_result.json`, which was keyed by AR&R class, carried no run
identifier, and was written by no script in the repo.
