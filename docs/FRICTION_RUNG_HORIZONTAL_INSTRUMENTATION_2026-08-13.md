...
**Stale provenance string in existing artifacts.** Every arm JSON written before this
correction carries `arm_provenance.friction_source` naming `sim_standing.py:132`. That
line is **triangle-rasterisation arithmetic**; the gated floor is `:210-211`. The *value*
0.55 was always right. Fixed in the script; the on-disk strings are not retro-edited.
