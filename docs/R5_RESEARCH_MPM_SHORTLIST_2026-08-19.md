...
```
**Trap: the registry record is malformed.** It returns a single author with an empty
...
| GIMP 2004 | the interpolation scheme in near-universal use, including in warpmpm-family solvers |
...
2. Whether warpmpm actually implements GIMP shape functions I did **not** check in
   the solver source. If it uses linear or cubic B-spline weighting instead, GIMP is
   still relevant background but the wording should change.
