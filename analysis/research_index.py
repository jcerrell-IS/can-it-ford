...
def build(export_dir: str = "", built: str = "") -> dict:
    ...
    return {
        # Was hardcoded, so every rebuild misdated itself as the original
        # build. Caught while wiring the export path. Kept from the corpus-bib
        # branch: a hardcoded date here is the same class of defect as a
        # hardcoded search list.
...
def report_ingest_audit(export_dir: str) -> int:
    ...
    print("INGEST AUDIT. What --build can reach, not what exists.\n")
    ...
    print("  defect one level up: it cannot notice a search added after it was")
    print("  written. IT IS ALREADY KNOWN STALE. data/deep_searches/MANIFEST")
    print("  records n_searches 27 from a workspace pull dated 2026-08-23, and")
    print("  the directory holds 28 searches as of 2026-08-24, against the")
    print(f"  {len(WORKSPACE_DEEP_SEARCHES)} in the constant below. Re-derive it in a")
...
def main() -> int:
    ...
    # RENAMED FROM --source-audit ON 2026-08-24, and the rename is the whole
    # reason this landing needed a decision. Both branches independently
    # declared a --source-audit and they are DIFFERENT CHECKS: the one above
    # is a CI GATE that exits 1 when a completed search reaches the corpus by
    # no route, and is the name CLAUDE.md and the corrections register already
    # cite; this one is a REPORT of what --build can reach from the export
    # directory. Neither was dropped. This one took the new name because it
    # has no external citers, and --ingest-audit follows the branch's own
    # --<noun>-audit convention and pairs with --ingest-check, which validates
    # a single export where this audits the whole ingest layer.
    ap.add_argument("--ingest-audit", action="store_true",
                    help="what --build can actually REACH, versus what the "
                         "workspace holds. Reports the blindness rather than "
                         "a clean eight-of-eight. Renamed from --source-audit "
                         "2026-08-24 to resolve a collision; --source-audit is "
                         "the CI gate, this is the reachability report.")
    ...
    if a.ingest_audit:
        return report_ingest_audit(a.export_dir)
    ...
    if a.query:
        # AUTHORS ARE MATCHED, AND THAT IS A FIX, NOT A FLOURISH.
        # This clause read `title or abstract` until 2026-08-19, so an
        # author-name query returned a STRUCTURALLY FALSE ZERO: the field being
        ...
        # 10.1111/jfr3.12828, the project's closest prior art. The consequence
        # is recorded on both branches and is the same event: one session
        # relayed "none of the six closest prior-art DOIs is in the corpus" to
        # three sessions, two of which acted on it. All six were present.
        ...
        # paraphrase, and for any record without an abstract it is title,
        # authors and journal only. Roughly a third of the corpus has no
        # abstract, so for those records a topic query can only match words in
        # the title. A zero here is weak evidence of absence and the tool
        # should say so rather than let the reader infer certainty from an
        # empty list. The `journal` field is included in the predicate below
        # because the add-ci-checks side added it; the caveat text names every
        # field actually searched, so the two cannot drift apart silently.
        _no_abs = sum(1 for r in pool if not r.get("abstract"))
        sys.stderr.write(
            f"[--query is a literal substring match over "
            f"title+abstract+authors+journal. "
            f"{_no_abs} of {len(pool)} searched records have NO abstract and were "
            f"matched on title, authors and journal only. A paraphrase will "
            f"miss. Do not read 0 matches as absence without a second route.]\n")
        ...
        if not sel:
            ...
            print("  --query never matches a DOI or a method tag, and on any")
            print("  ref older than 2026-08-19 it never matched an author")
            print("  either, which is how three sessions concluded this corpus")
            print("  lacked its own prior art while it held all six DOIs.")
            ...
            print("    python3 analysis/research_index.py --ingest-audit")
            print("  --source-audit exits 1 while any completed deep search")
            print("  reaches the corpus by no route, and --ingest-audit shows")
            print("  what --build can reach at all. RUN THEM RATHER THAN")
            print("  QUOTING A RATIO FROM THIS TEXT: the reached-versus-total")
            print("  figure is scope-sensitive and has moved more than once,")
            print("  so a number hardcoded here would be stale on arrival.")
            return 0
    if a.doi:
        d = a.doi.lower().strip()
