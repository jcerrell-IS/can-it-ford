...
def equal_density_masses(rho_ref):
    ...
    This is the control that separates geometry from mass. Arm M pairs each hull
    with its own vehicle's mass, so a difference between vehicles could be caused
    by either. Holding density fixed leaves size and shape as the only difference,
    which makes "does the ordering follow displaced volume or mass" falsifiable
...
def main():
    ...
    print("  Same three hulls, same matched dx, identical bulk density. Whatever")
    print("  ordering survives this cannot be attributed to mass.")
