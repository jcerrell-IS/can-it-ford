...
# =============================================================================
# FE-DENSITY VARIANT OF THE CANONICAL DRIVER. NEW FILE. sim_standing.py IS NOT
# EDITED, so the provenance of the 17 gated runs is untouched.
#
# With --fe-density-profile absent this takes the ORIGINAL code path exactly:
# vehicle particles unsorted, one set_material_range call. That is the control
# arm, and it must reproduce sim_standing.py.
#
# WHAT IT CHANGES. solidify_watertight fills the hull uniformly, so the CG lands
# at the geometric centroid, +61.0 mm above the FE model's own CG. That FE CG
# reproduces the CCSA validation report (DOI 10.13021/G8JS5D slide 7) to 0.6 mm
# once the companion set file's *ELEMENT_MASS_PART is included; omitting it loses
# 228.50 kg, 20.8 percent of the Yaris, including 16.0 kg sitting on the ROOF.
#
# NOT WIRING INERTIA. CLAUDE.md item 4 forbids writing the tabulated tensor in as
# a parameter and params_check.py check_inertia_wired() enforces it. No tensor is
# written. Only density versus height; the solver still derives CG and inertia
# from its own particle cloud, and total mass is renormalised to the wired --mass
# exactly, so every gate that reads mass is unaffected.
#
# PUBLIC API ONLY. set_material_range takes an INDEX range, so vehicle particles
# are sorted by z at seeding and each band becomes a contiguous range. Sorting at
# LOAD is safe; the solver forbids sorting MID-RUN only for index-paired dumps.
# additional_material_params was rejected: it selects by position and does not
# filter by material, so it would also re-densify water under the vehicle and in
# the wheel wells.
# =============================================================================
...
class StandingFloodScene:
    def __init__(self, vehicle, depth, velocity, vehicle_mass, n_grid=64,
                 ...
                 seed=0, inflow_len=1.5, fe_profile=None):
        ...
        # Height-sorting makes each density band a contiguous index range, which
        # is what set_material_range accepts. Only when a profile is supplied, so
        # the control arm keeps the original particle ordering.
        self.fe_profile = fe_profile
        if fe_profile is not None:
            truck = truck[np.argsort(truck[:, 2], kind="stable")]
        ...
        if fe_profile is None:
            s.set_material_range(self.n_water, self.n_total, "rigid", obj_id=0,
                                 density=vehicle_density)
            self.fe_bands_applied = 0
        else:
            zt = np.asarray(truck[:, 2], dtype=np.float64)
            z0, z1 = float(zt.min()), float(zt.max())
            span_z = z1 - z0
            per = np.full(len(zt), float(vehicle_density), dtype=np.float64)
            bands = fe_profile["bands"]
            for bi, b in enumerate(bands):
                lo = z0 + b["z_frac_lo"] * span_z
                hi = z0 + b["z_frac_hi"] * span_z
                m = ((zt >= lo) & (zt <= hi + 1e-9)) if bi == len(bands) - 1 \
                    else ((zt >= lo) & (zt < hi))
                if m.any():
                    per[m] = float(b["density_kg_m3"])
            per *= (float(vehicle_density) * len(zt)) / per.sum()
            edges = np.flatnonzero(np.diff(per) != 0.0) + 1
            starts = np.concatenate([[0], edges]).astype(int)
            ends = np.concatenate([edges, [len(per)]]).astype(int)
            for a_, b_ in zip(starts, ends):
                s.set_material_range(self.n_water + int(a_), self.n_water + int(b_),
                                     "rigid", obj_id=0, density=float(per[a_]))
            self.fe_bands_applied = len(starts)
            cg = float((zt * per).sum() / per.sum())
            print("FE_DENSITY bands=%d ranges=%d rho_min=%.3f rho_max=%.3f "
                  "rho_uniform=%.3f mass_kg=%.5f cg_above_hullbase=%.6f "
                  "uniform_cg_above_hullbase=%.6f"
                  % (len(bands), len(starts), per.min(), per.max(),
                     float(vehicle_density), per.sum() * (h ** 3),
                     cg - z0, float(zt.mean()) - z0), flush=True)
...
def main():
    ...
    p.add_argument("--fe-density-profile", default="",
                   help="JSON from analysis/fe_vehicle/fe_density_profile.py. "
                        "Absent = uniform density, reproducing sim_standing.py.")
    ...
    scene = StandingFloodScene(v, depth=a.depth, velocity=a.velocity,
                               ...
                               fe_profile=(json.load(open(a.fe_density_profile))
                                           if a.fe_density_profile else None))
