...
def classify_one(run_dir: Path, prov: dict):
    ...
    return {
        ...
        # floor_friction is written by sim_standing.py:511 and was previously dropped
        # here, which left the mu-ladder and (dx,mu) results with mu recoverable only
        # from the run LABEL. A friction sweep whose friction is not a column is not
        # reproducible. n_carved (:510) is needed for any correct P-2 baseline.
        "floor_friction": float(summary["floor_friction"]),
        "n_carved": int(summary["n_carved"]),
        "frames": int(summary["frames"]),
        "bulk_modulus": float(summary["bulk_modulus"]),
        "sound_speed_ms": float(summary["sound_speed_ms"]),
        # --- vehicle ---
        "mass_kg": mass,
