...
def setup(p, focal, view_radius, az, elev, zfloor, ssao_frac):
    ...
    # SHADOWS DISABLED 2026-07-26. VTK's shadow-map pass has no translucent stage,
    # so it silently drops every actor with opacity < 1.0. That is why the water
    # isosurface was invisible in all 90 frames of hero_g64_m1100.mp4 despite being
    # built correctly (222588 faces, 0 open edges, volume +3.0%). Verified by three
    # single-frame probes: shadows+0.62 = no water; shadows+1.0 = opaque water;
    # no-shadows+0.62 = translucent water, road visible through it. Depth peeling
    # (12 peels) does NOT fix it.
    # p.enable_shadows()
