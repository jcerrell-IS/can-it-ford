...
12. Output is binary. simulation/failure_modes.py implements
    STUCK/SLIDE/TOPPLE/FLOAT but is never wired into the 17-run
    pipeline. renders/yaris_render_s1/failure_modes_result.json has no
    run identifier and is written by no script in the repo.
    docs/four_rung_ladder.md:136 and four_rung_ladder_GRIDAWARE.md:136
    cite it as independent confirmation, which it cannot be. Do not
    repeat that claim.
