...
The coupled sweep supporting AR\&R-class-linked results is the gated inventory in \texttt{data/all\_runs\_inventory.csv}: 17 runs on the watertight Yaris hull (Section~\ref{sec:approach}C), not \texttt{track1\_sweep\_v2}, which is superseded and excluded per project policy. It comprises a 3$\times$3 mass-by-resolution block (9 runs, masses 1100/1609/2337\,kg directly matching AR\&R's small passenger/large passenger/large 4WD classes, no class-mapping needed, at \texttt{n\_grid} 48/64/96) fixed at depth 0.30\,m and velocity 1.5\,m/s, plus a three-point depth sweep and a five-point velocity sweep at \texttt{n\_grid}=64. Floor friction is held uniform at 0.55 across all 17 runs. 7 of the 17 runs exceed a 10\% particle-passthrough gate; these are flagged, not excluded. \FLAG{An L1-vs-L2 agreement rate from this sweep still requires the L2 drift threshold, which Table~\ref{tab:ladder} already flags as uncited; report displacement magnitudes only until that is resolved.}
