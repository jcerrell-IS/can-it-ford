...
% ---------------------------------------------------------------
% FLAG / PLACEHOLDER render in red so an unresolved item cannot be
% missed in the compiled PDF. As of the submission-close pass on
% July 30, 2026 there are zero uses of either: each was resolved
% against a primary source or rewritten as a limitation in the
% paper's own voice. The macros are kept so that re-checking stays a
% one-line grep for either macro name followed by an opening brace.
% This comment deliberately does not spell that pattern out, so that
% the check matches real uses only. Any hit means the draft is not
% submission-ready.
% ---------------------------------------------------------------
...
\midrule
L0 & Depth & Fixed depth cutoff, independent of velocity or vehicle class. NWS publishes two figures: 6\,in ($\approx$0.15\,m) of fast-moving water knocks over an adult, and 12\,in ($\approx$0.30\,m) carries away most cars \cite{nws_tadd}. We adopt the 0.15\,m pedestrian figure as a deliberately conservative vehicle bound rather than NWS's own 0.30\,m vehicle figure, since vehicle stability testing places small-car loss of traction close to 0.30\,m rather than well above it \cite{smithmodrafelder2019,azhar2023}. \\
...
L2 & MPM & Genesis rigid-MPM simulation; NO-FORD when lateral drift exceeds 0.05\,m. That figure is a conservative numerical onset-of-motion detector internal to the solver, marking where the rigid body has left its initial pose by more than solver noise. It is not an empirical stability limit and is not offered as one; the sliding physics it is meant to catch is characterized experimentally elsewhere \cite{shah2018instability,xia2014criterion}. \\
\bottomrule
\end{tabular}
\end{table}

The L0 bound is consequential rather than cosmetic. Across the 70 (depth, velocity) scenarios of \texttt{data/scenario\_sweep.csv}, a 0.15\,m cutoff permits crossing in 7; NWS's own 0.30\,m vehicle figure would permit 21, three times as many.
...
A first-principles force balance motivates why L1's scalar hazard measure is expected to be insufficient. Fig.~\ref{fig:force} plots lateral hydrodynamic drag force against maximum available static friction at several water depths (left), and the resulting critical velocity above which drag is predicted to exceed friction, as a function of depth (right). The drag term is the standard quadratic form $F_D = \tfrac{1}{2}\rho C_d A V^2$, and the friction limit is $\mu N$ with the normal force $N$ reduced by buoyancy as depth rises. One limitation of this figure should be stated plainly: the drag coefficient and frontal area behind the plotted curves are not recoverable from a retained generator script, so we do not report them as values and do not claim they match the friction range cited elsewhere in this paper. The figure is used for the shape of the drag-versus-friction argument, not for the numbers on its axes.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{force_balance.jpg}
\caption{Analytical force-balance calculation, not simulation output. Left: lateral drag force versus flow velocity at four water depths, against the maximum static friction limit (dashed). Right: the resulting critical velocity, above which drag exceeds friction, as a function of depth. The legend reports $N=0$ and $F_{\text{fric}}=0$ at 0.30, 0.45, and 0.60\,m. Those zeros are a real buoyancy result, not missing values: at 0.30\,m this model's buoyant force already reaches 15.99\,kN against a vehicle weight of 10.79\,kN, so normal force is zero and the friction limit $\mu N$ vanishes with it. The right panel corroborates this independently, with critical velocity falling to zero near 0.19\,m. That flotation depth is a property of the model's displacement assumption, which treats the vehicle as a solid prism over roughly three quarters of its bounding-box footprint. The real hull encloses 3.5427\,m\textsuperscript{3} against the 10.7457\,m\textsuperscript{3} prism this model assumes, a fill factor of only 0.33, so the prism assumption overstates displacement in shallow water and floats the vehicle at about 0.20\,m. That prism is the nominal reference bounding box, 4.30\,$\times$\,1.70\,$\times$\,1.47\,m from the vehicle specification, and not the hull's own measured extent: the mesh measures 4.2826\,$\times$\,1.7464\,$\times$\,1.5180\,m, enclosing 11.3533\,m\textsuperscript{3}, against which the fill factor is 0.312. The two differ by about 6\%, which does not change the argument here but is the reason the prism figure should not be read as a measurement of this hull. That is below even the 0.30\,m depth cap AR\&R assigns to its Small Car class \cite{shand2011arr}, so the model reaches flotation before the empirical guideline reaches its own depth limit. This figure should therefore be read as establishing the \emph{shape} of the drag-versus-friction argument, not as a calibrated flotation prediction.}
\label{fig:force}
\end{figure}

\subsection{Vehicle and Scene Representation}
The vehicle asset used to find and root-cause this project's grid-resolution solidification defect is a rigid body solidified from a surface Gaussian splat (\texttt{truck\_trimmed.ply}, 191{,}107 points, no interior), used because no watertight simulation-ready mesh existed in the pipeline at the time. Three vehicle groupings (sedan, SUV, pickup) are each a single rescaled base silhouette, stretched to bounding-box dimensions and mass properties taken from individual measured vehicles in NHTSA's inertial-parameter database \cite{heydinger1999sae}. That database reports parameters measured one vehicle at a time; it does not publish a matched sedan/SUV/pickup class set, and the grouping into three classes here is ours, not its. Its coverage also ends in November 1998, so it constrains this diagnostic asset only and is not a source for the watertight hull introduced below, which it does not contain. This is stated explicitly as a limitation, not glossed over: all three groupings share one silhouette, so cross-group comparisons involving this asset reflect scaled mass and footprint, not distinct vehicle geometry.
...
The candidate fix this diagnosis points to, moving to a true watertight mesh rather than re-tuning the solidification pitch, is what the project adopted for vehicle representation tied to AR\&R's own classes. A crash-validated, watertight 2010 Toyota Yaris hull \cite{ccsa2016yaris} (\texttt{yaris\_coarse\_v1l\_watertight.ply}; mass 1100\,kg from the LS-DYNA deck header, \texttt{Version 1l, 1100 kg}) replaces the single rescaled silhouette with one real, watertight geometry, and vehicle class is expressed as an explicit mass override on that same geometry rather than as three separate meshes (\texttt{renders/yaris\_render\_s1/gates\_both\_scenarios.py}): \texttt{small\_passenger} at 1100\,kg, \texttt{large\_passenger} at 1609\,kg, and \texttt{large\_4wd} at 2337\,kg, taking the kerb weight of each of AR\&R's three classes \cite{shand2011arr}. The mass is taken from those classes; the geometry is not. AR\&R defines its classes jointly on length, kerb weight and ground clearance, and this hull measures 4.2826\,m, which fails the length criterion for large passenger ($>$4.3\,m) and for large 4WD ($>$4.5\,m) at every mass tested. The three runs are therefore a controlled mass-sensitivity study on one hull rather than three vehicles, and are reported as such. This also settles the class-mapping question in Fig.~\ref{fig:threeclass}: AR\&R's classes do not map onto the sedan/SUV/pickup grouping used for the diagnostic asset in Table~\ref{tab:vehicle} at all, and they are kept deliberately distinct from the silhouette-based diagnostic above. This mass-override asset and method, not the silhouette splat, is what any AR\&R-class-linked simulation result in this paper is run on.
...
Structure-from-motion reconstruction (COLMAP) is complete for one real scene (``Drain A''), confirmed via a populated sparse point cloud and camera pose set on TACC Vista scratch storage. Gaussian-splat training from that reconstruction runs separately on LS6 and is not complete, so Drain A is not yet a simulation-ready splat and no result reported here is drawn from one. A second scene (``Drain B'') has not been captured. The reconstruction half of the pipeline in Fig.~\ref{fig:pipeline} is accordingly designed and only partially built, and every simulation result below runs on the watertight vehicle hull in synthetic surroundings rather than on a reconstructed real scene.

\subsection{Synthetic-Geometry Pilot Study}
An early parametric comparison of L1 against a Genesis SPH solver (\texttt{data/l2\_results\_from\_wandb.csv}) covers 9 unique (depth, velocity) conditions. L1 and L2 agreed at 5 of 9 (55.6\%). The 4 divergence points, where L1 returned FORD but L2 returned NO-FORD, were (0.30\,m, 1.5\,m/s), (0.15\,m, 1.5\,m/s), (0.30\,m, 1.0\,m/s), and (0.30\,m, 2.0\,m/s). An earlier draft of this subsection reported 23 pairs, 16 divergence points and 30.4\% agreement. Those numbers do not match this file and were never traced to a source before being written, so they are superseded by the figures above, which are read directly from the CSV. A separate file, \texttt{data/phase\_space\_results.csv}, does contain 23 unique (depth, velocity) pairs, matching the old claim's pair count, but it carries a single verdict column with no corresponding L1 value, so no agreement rate can be computed from it. 15 of its 31 rows also share a condition with another row under a different verdict: three separate (0.30\,m, 1.5\,m/s) runs return FORD, NO-FORD and NO-FORD at three different displacements. It is not run-labelled cleanly enough to use as it stands, so only the 9-pair result is reported.

A separate 4-point friction sweep at the (0.30\,m, 1.5\,m/s) divergence point (\texttt{data/mu\_sweep\_results.csv}) shows lateral displacement of 0.399, 0.3957, and 0.3953\,m at coupling friction $\mu = 0.3, 0.5, 0.7$ respectively, essentially invariant across that range. At $\mu = 0.0$, displacement drops to 0.3283\,m, a real difference, not noise. The invariance therefore holds from $\mu = 0.3$ to $0.7$, not across the full range from $\mu = 0.0$; an earlier draft described it as invariant from 0.0 to 0.7, which overstates it. The qualitative point stands, that a scalar hazard measure cannot represent this directional force, but this is four points at a single condition and nothing here is generalized beyond it.

Fig.~\ref{fig:l2schematic} is accordingly presented as an explicitly labelled schematic rather than a plotted result, carrying a ``SCHEMATIC'' watermark and showing only the conceptual shape of the claim, until it can be rebuilt from the verified 9-point dataset above or from a larger confirmed re-run.
...
\label{subsec:sweep}
The coupled sweep supporting AR\&R-class-linked results is the gated inventory in \texttt{data/all\_runs\_inventory.csv}: 17 runs on the watertight Yaris hull (Section~\ref{sec:approach}C), not \texttt{track1\_sweep\_v2}, which is superseded and excluded: that earlier 36-run sweep used a rescaled box proxy of 1390\,kg enclosing 4.7352\,m\textsuperscript{3}, against the real hull's 3.5427\,m\textsuperscript{3}. It comprises a 3$\times$3 mass-by-resolution block (9 runs, masses 1100/1609/2337\,kg taken from the kerb weights of AR\&R's small passenger, large passenger and large 4WD classes, at \texttt{n\_grid} 48/64/96) fixed at depth 0.30\,m and velocity 1.5\,m/s, plus a three-point depth sweep and a five-point velocity sweep at \texttt{n\_grid}=64. As noted in Section~\ref{sec:approach}C, only the mass varies across that block: the single 4.2826\,m hull fails AR\&R's length criterion for both upper classes at every mass, so the block is a controlled mass-sensitivity study on one geometry and not a comparison of three vehicle classes. Ground-plane friction is held uniform at 0.55 across all 17 runs; this is the friction parameter passed to the solver's \texttt{add\_plane} call, a boundary-condition setting on the MPM floor, and not a measured tire-road friction coefficient. 7 of the 17 runs exceed a 10\% particle-passthrough gate, reaching 15.9\% at 3.0\,m/s; these are flagged, not excluded. Water-layer depth resolution also varies across the block, from three layers at the coarsest grid to six at the finest, so the coarse-grid rows are the least trustworthy independent of their passthrough figure. This sweep is reported as displacement magnitudes and not as an L1-vs-L2 agreement rate. Turning displacements into L2 verdicts requires the 0.05\,m drift threshold, which Table~\ref{tab:ladder} defines as a solver-internal onset-of-motion detector rather than an empirical stability limit. An agreement rate computed on top of it would inherit that status while reading as an empirical validation, so it is withheld here.
...
\item \textbf{A second independently-sourced real scene.} The FRED flooded-road dataset \cite{fred2026} captures five flooded locations with a full autonomous-vehicle sensor stack, camera, LiDAR, IMU and RTK GNSS, recorded both during and after flooding, and is publicly released with semantic water-hazard labels. Sourcing a second scene externally would decouple the pipeline's evaluation from our own capture conditions.
