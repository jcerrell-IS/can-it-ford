...
% that were executed with warpmpm. Re-clear with Kumar before submission.
% =================================================================
\begin{abstract}
Flooded roads are a leading cause of vehicle fatalities, killing drivers who cannot judge how deep the water is, how fast it moves, or what the road beneath it is doing. Automated vehicles inherit the same gap, and a world model that predicts only appearance can render an outcome that is physically wrong. Physically viable world models address this by preserving the latent physics that governs the outcome, but leave open how much fidelity a given query requires. We instantiate that abstraction-selection orchestrator for one narrow query: can this vehicle ford this flooded road? The full pipeline reconstructs a scene by Gaussian splatting and bridges it to particles following PhysGaussian; that front end is designed and not yet built, so every result here uses a flood domain specified parametrically in depth and velocity. We compare three fidelity levels on a common depth-velocity grid: a static depth cutoff, the Australian Rainfall and Runoff depth-velocity criterion, and a coupled rigid-body Material Point Method simulation of a watertight Toyota Yaris hull. Across 70 conditions, applying AR\&R's depth and velocity limits jointly rather than its hazard product alone reclassifies 23 cells from FORD to NO-FORD, all in the same direction, and vehicle class alone changes the verdict in 12. In a 9-condition coupled pilot the two levels agree at only 5, and all four disagreements run one way: the criterion permits where the simulation refuses. The minimal sufficient abstraction is therefore query-dependent, and the cheap criterion fails asymmetrically, permitting crossings the coupled model rejects.
\end{abstract}
