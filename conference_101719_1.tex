...
% =================================================================
% ABSTRACT
% Rewritten July 31 to IEEE length norm and to report results.
% Supersedes the June 18 CanItFord_Abstract_FINAL.docx wording, which
% ran 296 words, was written in expectation tense, claimed a splat
% reconstruction Section I disclaims, and credited Genesis for runs
% that were executed with warpmpm.
%
% Second pass, same day, restoring the approved framing and fixing the
% one accuracy defect the first rewrite introduced:
%   the 9-condition agreement was presented as coming from the coupled MPM
%   Yaris configuration. It is the Genesis SPH pilot on synthetic geometry
%   (Section IV, Synthetic-Geometry Pilot Study), and the body reports NO
%   L1-versus-L2 agreement rate for the 17-run Yaris sweep at all.
% The "vehicle class alone ... 12" claim was briefly challenged as unsourced
% and is NOT: it is Fig 3's headline, 5 + 7 class-sensitive cells of 70.
% It is stated only in the figure artwork, never in the body prose, which is
% worth fixing in Section IV independently.
% Opening, narrow-question framing, and closing contribution sentence are
% carried over from the June 18 approved text. 250 words.
% Re-clear with Kumar before submission.
% =================================================================
\begin{abstract}
Flooded roads are a leading cause of vehicle fatalities, killing drivers who cannot judge how deep the water is, how fast it moves, or what the road beneath it is doing. Automated vehicles inherit the same gap: a world model predicting only appearance can render an outcome that looks plausible but is wrong. Physically viable world models preserve that latent physics, but leave open how much fidelity a query requires. This project asks a narrow question, can an autonomous vehicle ford a specific flooded road, and identifies the simplest abstraction sufficient to answer it, not the most detailed simulation available. We compare three fidelity levels on a depth-velocity grid: a static depth cutoff, the Australian Rainfall and Runoff (AR\&R) criterion, and a coupled rigid-body Material Point Method simulation of a watertight Toyota Yaris hull. The splatting front end and PhysGaussian bridge are designed and not yet built, so every flood domain here is parametric. Across 70 conditions the depth cutoff admits 7 crossings and the joint criterion 14; tightening the rule and the class together reclassifies 23 cells to NO-FORD with none in the opposite direction, and vehicle class alone decides the verdict in 12. In a 9-condition pilot on synthetic geometry, run with SPH rather than the coupled configuration, the two agree at only 5, and all four disagreements run one way: the criterion permits where the simulation refuses. The minimal sufficient abstraction is therefore query-dependent, and this work delivers a reconstruct-to-decide pipeline instantiating query-conditioned physically viable world models.
\end{abstract}
...
3D Gaussian Splatting (3DGS) reconstructs a photorealistic, explicit scene representation from ordinary multi-view video \cite{kerbl20233dgs}. PhysGaussian extends this representation with physical simulation by treating each Gaussian kernel's covariance as a seed for a continuum Material Point Method (MPM) particle volume, allowing a trained splat to drive a physics simulation directly rather than only a renderer \cite{xie2023physgaussian}. The pipeline is designed around Genesis, an open-source, solver-agnostic physics engine \cite{genesis2024}. We are not aware of a publicly available, drop-in pipeline carrying flood video through to a Genesis MPM simulation: PhysGaussian's own reference implementation targets a Warp MPM solver rather than Genesis, so that bridge has to be built rather than adopted, and it is not built yet.
...
The uniform reporting standard for grid refinement studies is Roache's \cite{roache1994}, and the ASME procedure for converting such a study into a discretization-uncertainty band is Celik et al.\ \cite{celik2008}. \emph{Neither is applied here, and no Grid Convergence Index is reported.} Both require a monotone response in the asymptotic range, from which an apparent order of convergence can be extracted; the block above is non-monotone in resolution, so no apparent order can be computed and any GCI derived from these three grids would be a number without the property it claims to measure. What this study reports is strictly weaker, and is stated as such: the FORD/NO-FORD \emph{verdict} is invariant across all three grids tested, \texttt{n\_grid} 48, 64 and 96, while the underlying displacement \emph{magnitude} is not. The verdict-invariance is the reportable result. The magnitude is not, and the two must not be conflated.

Loss of convergence under grid refinement at fixed particles-per-cell is a documented property of explicit MPM rather than a defect specific to this setup. Bai and Schroeder give a stability analysis of explicit MPM \cite{baischroeder2022}, and Sun, Shinar and Schroeder derive the effective time step restrictions that govern it \cite{sunshinarschroeder2020}. Both bear directly on whether a refinement sequence at fixed particle count can be expected to lie in the asymptotic range at all, which is the precondition the two paragraphs above identify as unmet.
...
The synthetic-geometry pilot demonstrates, as a proof of concept, that a scalar depth-velocity hazard measure can disagree substantially with a coupled physical simulation, and that the disagreement is not closed by coupling friction: at the one divergence point where friction was swept, no value in the tested range changes the verdict. Whether the disagreement is structural rather than a calibration artifact remains open, since the floor-friction assumption is uncontrolled, as noted in Section~\ref{sec:results}. This supports treating abstraction selection as query-dependent rather than assuming any single fixed-fidelity model is correct everywhere in the operating envelope.
