...
- STALE PARAMETER, live-verified: `designsafe-staging/scripts/can_it_ford_L2.py` hardcodes `rho=604` twice, at line 40 (`gs.materials.Rigid(needs_coup=True, coup_friction=0.55, rho=604)`) and line 131 (the result record). The live simulation reads `VEHICLE_RHO = 115.7` at `simulation/can_it_ford_L2_mpm.py` line 27 and passes it as a variable rather than a literal. The staged copy is stale by exactly the discrepancy the known-error register already documents: 604 was left over from a prior box size after a sedan-scale resize.
- OWNERSHIP: pane C2 is fixing this now. That file is C2's; no other pane should edit it.
- CONSEQUENCE, do not lose this: anything already submitted to or staged for DesignSafe PRJ-6388 out of this directory carries `rho=604`. A 604 vs 115.7 vehicle density is a different buoyancy regime, so any figure or result in that package is UNVERIFIED until regenerated against the live value. Nothing was pulled, edited, or resubmitted by this pane.
