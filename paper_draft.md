...
Methods. A flooded-road scene is reconstructed in 3D using Gaussian splatting, following the PhysGaussian framework in which Gaussian kernels seed a continuum particle representation for a Material Point Method (MPM) simulation. Three levels of physical fidelity are compared: (0) a static depth threshold derived from published flood-vehicle stability data (Smith, Modra & Felder 2019), (1) a depth-velocity stability criterion from the Australian Rainfall and Runoff guidelines, and (2) a coupled simulation in which a rigid vehicle body interacts with weakly compressible floodwater. The coupled results reported here use box-proxy vehicle geometry, with per-class mass, size, displaced volume, and drag area matched to a real vehicle class rather than a real vehicle mesh: a Genesis SPH pilot establishes the abstraction-ladder structure, and an mpm-engine (kks32/mpm-engine) MPM sweep extends it across vehicle classes. The intended target, a rigid vehicle mesh immersed in Genesis MPM floodwater, is not yet numerically stable; it fails with a reproducible particle-to-grid crash, produced none of the reported results, and is documented here as ongoing work rather than glossed. All simulations run on NVIDIA A100 GPUs at TACC.
...
This corrects a classification error in an earlier draft, which assigned the vehicle to the Small Car class (D times V less than or equal to 0.30 m2/s). That Small Car classification is correct only for the real 1078 kg[^yaris-mass], 4.30 m Toyota Yaris geometry, which is a separate and not-yet-validated capability: the --vehicle yaris path in ford_sweep_driver.py was wired in this week and is still under validation after a mass-assignment bug was found. It does not describe the box-proxy sedan and pickup that produced the cited results.
...
[^yaris-mass]: 1078 kg is this specific NCAC finite-element mesh's own modeled mass. The 1100 kg figure commonly cited for the 2010 Toyota Yaris is the MASH nominal test-class standard, not this mesh's modeled weight.
...
### 5.2 The L1-to-L2 Divergence Mechanism

The depth-velocity product H = D times V is a scalar hazard index, and its structural limit is that it carries no term for the normal force acting between the vehicle and the road. Sliding resistance is frictional, proportional to that normal force, and the normal force is not the vehicle's dry weight but its weight less the buoyant uplift of the displaced water. As depth increases toward the vehicle's flotation point, buoyancy grows and the effective normal force collapses, so the friction available to resist lateral drag degrades faster than any function of D times V alone can express. A scalar that multiplies depth and velocity treats added depth as monotonically raising hazard through the product, but it cannot represent the separate, accelerating loss of frictional anchorage that the same added depth produces by reducing the normal force. This is why L1 and L2 diverge in the fast-shallow-flow regime: L1 reports a sub-threshold H and returns FORD, while L2, in which buoyancy-reduced normal force and the resulting persistent lateral drag emerge from the coupled dynamics rather than being prescribed, resolves the continuous momentum accumulation that carries the vehicle off the roadway. The divergence is a property of the abstraction, not of the particular threshold chosen. The Australian Rainfall and Runoff report that supplies the L1 criterion is itself explicit on this point, presenting its per-class D times V limits as "Draft, interim, informal" values that are "unlikely reliable enough to be adopted permanently as safety criteria" (Shand et al. 2011, Table 3). Treating a scalar hazard index as a sufficient traversability verdict inherits exactly the limitation its own authors flagged; recovering the buoyancy-normal-force coupling that the index omits is the specific work the L2 model performs.
