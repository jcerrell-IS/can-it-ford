...
def run_c1(n_grid, rho_box=600.0, depth_cells=18.0, box_bottom_cells=8.0,
    ...
    for i in range(measure_substeps):
        tank.project_water()
        tank.solver.step(tank.dt, 1)
        cxyz = tank.solver.rigid_state()["com"]
        print("com_frame", i, cxyz[0], cxyz[1], cxyz[2], flush=True)
        ts.append((i + 1) * tank.dt)
        vs.append(float(tank.solver.rigid_state()["v"][2]))
...
def run_c1b(n_grid, rho_box=600.0, depth_cells=18.0, submersion_frac=0.80,
            settle_frames=1200, measure_substeps=160, device="auto", seed=0):
    """C1b: C1 with the cube PARTIALLY submerged. The free surface is the only change.

    C1 holds the cube entirely below the free surface, so the only interface touching the
    body is water-against-solid. C1b lifts the SAME cube in the SAME tank at the SAME
    water depth, the same rho_box, the same still water, the same floor plane and four
    slip walls (restitution 0.0, friction 0.0, unchanged), the same settle procedure and
    the same measurement window, until a stated fraction of it sits below the surface.
    The free surface then cuts the body. Nothing else moves. No boundary planes are added
    or removed relative to C1, so this isolates the free surface and nothing else.

    SUBMERSION IS SET BY box_bottom_cells, WHICH IS DERIVED HERE AND NOT ACCEPTED FROM
    THE CALLER, so the stated fraction cannot silently disagree with the geometry. See
    box_bottom_cells_for_submersion for the exact mapping.

    THE ANALYTIC REFERENCE IS NOT C1's rho_w*V*g. A partially submerged cube displaces
    only its wetted part, so the reference is rho_w*g*L^2*h_sub, and the ideal release
    acceleration is

        a_ideal = g*(rho_w/rho_box * frac - 1)

    which is C1's g*(rho_w/rho_box - 1) only at frac = 1. The HEADLINE reference is built
    from the MEASURED settled surface, not from the requested fraction: the request is a
    placement target, the settled surface is what the body actually feels. The nominal
    version is reported alongside, never instead.

    DEGENERACY, recorded in the output rather than left for the reader to notice:
    a_ideal vanishes at frac = rho_box/rho_w, which is 0.60 at C1's rho_box = 600. That is
    the equilibrium floating draft, where the body is neutrally buoyant and a percent
    error against a_ideal is meaningless. The 0.80 default sits 0.20 clear of it and gives
    a_ideal = 3.27 m/s^2, exactly half of C1's fully submerged 6.54 m/s^2.
    """
    if not 0.0 < submersion_frac < 1.0:
        raise ValueError(
            f"submersion_frac={submersion_frac} must be strictly inside (0, 1). "
            f"frac >= 1 is C1 (use --variant c1); frac <= 0 is a dry body.")

    box_bottom_cells = box_bottom_cells_for_submersion(submersion_frac, depth_cells)
    dx = LIM / n_grid
    floor = 3.0 * DX_CANON
    z_b0 = floor + box_bottom_cells * DX_CANON
    if box_bottom_cells < 0.0:
        raise ValueError(
            f"submersion_frac={submersion_frac} at depth_cells={depth_cells} needs "
            f"box_bottom_cells={box_bottom_cells:.4f} < 0, i.e. the cube bottom below "
            f"the floor plane. Raise depth_cells or lower submersion_frac.")
    if z_b0 < 1.5 * dx:
        raise ValueError(
            f"cube bottom z={z_b0:.4f} m is inside the P2G edge guard 1.5*dx="
            f"{1.5 * dx:.4f} m at n_grid={n_grid}.")

    tank = BoxTank(n_grid, rho_box, depth_cells, z_b0, water=True,
                   device=device, seed=seed)
    settle = settle_pinned(tank, settle_frames)

    zs_settle, n_free_cols, iqr_settle = tank.column_surface()
    box_top = z_b0 + tank.length
    # MEASURED wetted height and fraction. clip: a surface above the cube top is full
    # submersion (frac 1), below the cube bottom is a dry body (frac 0). Either means
    # the run did not realise partial submersion and is flagged below, not silently used.
    h_sub = float(np.clip(zs_settle - z_b0, 0.0, tank.length))
    frac_realized = h_sub / tank.length
    # independent surface estimate: total water volume through the same still-water
    # balance the placement uses. Agreement between the two is a check on both.
    _d_area, zs_area = tank.draft_by_tank_area()
    h_sub_area = float(np.clip(zs_area - z_b0, 0.0, tank.length))
    ...
    # analytic references. _nom from the REQUESTED fraction, unsubscripted from the
    # MEASURED one. The measured pair is the headline.
    a_ideal = G * (RHO_W / rho_box * frac_realized - 1.0)
    a_ideal_nom = G * (RHO_W / rho_box * submersion_frac - 1.0)
    f_buoy_analytic = RHO_W * G * tank.length ** 2 * h_sub
    f_buoy_analytic_nom = RHO_W * G * tank.length ** 2 * (submersion_frac * tank.length)
    ...
    m = tank.mass
    neutral_frac = rho_box / RHO_W
    # a_ideal -> 0 at the neutral fraction, so a percent error against it diverges there.
    # Guard the division and publish the margin rather than printing a huge number.
    degenerate = abs(a_ideal) < 1e-9

    return {
        "variant": "C1b_partial_submersion_initial_acceleration",
        "differs_from_c1_by": "submersion only: the cube is raised so the free surface "
                              "cuts it. Water depth, rho_box, tank, floor plane, four "
                              "slip walls, settle procedure and measurement window are "
                              "C1's, unchanged.",
        # --- the stated submersion, explicit, never implicit -------------------------
        "submersion_frac_requested": float(submersion_frac),
        "submersion_frac_realized": frac_realized,
        "submersion_frac_realized_by_tank_area": h_sub_area / tank.length,
        "submersion_frac_shortfall": frac_realized - float(submersion_frac),
        "submerged_height_m": h_sub,
        "submerged_height_dx": h_sub / dx,
        "freeboard_m": tank.length - h_sub,
        "freeboard_dx": (tank.length - h_sub) / dx,
        "box_bottom_cells_derived": box_bottom_cells,
        "box_bottom_cells_source": "derived from submersion_frac by "
                                   "box_bottom_cells_for_submersion(); --box-bottom-cells "
                                   "is IGNORED for variant c1b",
        "partially_submerged_at_release": bool(0.02 < frac_realized < 0.98),
        "surface_estimator_iqr_m": iqr_settle,
        "surface_estimator_free_columns": n_free_cols,
        "surface_agreement_column_vs_area_m": zs_settle - zs_area,
        "geometry": tank.geometry(),
        "settle": settle,
        "settle_is_discard": not settle["settle_gate_met"],
        "surface_after_settle": zs_settle,
        "surface_after_settle_by_tank_area": zs_area,
        "box_top_z": box_top,
        "z_b_nominal_at_spawn": z_b0,
        "submersion_margin_dx": (zs_settle - box_top) / dx,
        # --- analytic reference, RECOMPUTED for the partial geometry -----------------
        "a_ideal": a_ideal,
        "a_ideal_nominal_from_requested_frac": a_ideal_nom,
        "a_ideal_reference_basis": "g*(rho_w/rho_box*frac - 1) at the MEASURED frac; "
                                   "this is NOT C1's g*(rho_w/rho_box - 1)",
        "a_ideal_c1_full_submersion_for_contrast": G * (RHO_W / rho_box - 1.0),
        "a_added_mass_asymptote_ca067": a_ideal / (1.0 + 0.67 * RHO_W / rho_box),
        "F_buoy_analytic": f_buoy_analytic,
        "F_buoy_analytic_nominal": f_buoy_analytic_nom,
        "F_buoy_analytic_c1_full_submersion_for_contrast": RHO_W * tank.volume * G,
        # --- degeneracy bookkeeping --------------------------------------------------
        "neutral_frac_a_ideal_zero": neutral_frac,
        "margin_to_neutral_frac": frac_realized - neutral_frac,
        "a_ideal_is_degenerate": bool(degenerate),
        "a_ideal_sensitivity_per_frac": G * RHO_W / rho_box,
        # --- measurement -------------------------------------------------------------
        "a_windows": windows,
        "a_headline_first3": a0,
        "a_late_window": a_late,
        "err_headline_vs_ideal_pct": (100.0 * (a0 - a_ideal) / a_ideal
                                      if not degenerate else float("nan")),
        "err_late_vs_ideal_pct": (100.0 * (a_late - a_ideal) / a_ideal
                                  if not degenerate else float("nan")),
        "F_buoy_from_a": m * (a0 + G),
        "err_F_pct": (100.0 * (m * (a0 + G) - f_buoy_analytic) / f_buoy_analytic
                      if f_buoy_analytic > 0 else float("nan")),
        "acoustic_transit_substeps": (tank.length / tank.sound_speed) / tank.dt,
        "t_series": ts,
        "v_series": vs,
        "diagnostics": tank.diagnostics(),
...
def main():
    ...
    p.add_argument("--variant", default="c2",
                   choices=["c0", "c1", "c1b", "c2", "c3", "c1sdf"])
    p.add_argument("--submersion-frac", type=float, default=0.80,
                   help="c1b only: fraction of the cube below the free surface. "
                        "box_bottom_cells is DERIVED from this and --box-bottom-cells is "
                        "ignored. a_ideal vanishes at rho_box/rho_w (0.60 at rho_box=600), "
                        "so values near there are degenerate and are flagged as such.")
    ...
    if a.variant == "c0":
        ...
        res = run_c1b(a.n_grid, a.rho_box, a.depth_cells, a.submersion_frac,
                      a.settle_frames, a.measure_substeps, a.device, a.seed)
