...
WHY THIS RUNG AND NOT A RE-RUN OF RUNG (b)
  ...
  three reasons the ladder cannot yet say, and only one of them is cheap to close:
    ...
    "Floor friction. This ladder walked restitution only. Its floor stays at BoxTank's
     friction=0.0; the 17 gated runs carry floor_friction=0.55 (sim_standing.py:132-133
     in the GATED DRIVER, sha256 5215c38b, preserved at
     renders/yaris_render_s1/_incoming/sim_standing.py; see register D8c).
  ...
  [The :132 in that quoted docstring is CORRECT, and an earlier note here calling it STALE
   was WRONG and is withdrawn. :132-133 is the floor add_plane in the driver that produced
   the 17 (sha256 5215c38b, 389 lines, preserved at _incoming/sim_standing.py). It is
   :210-211 only in the 2026-08-08 revision that later overwrote the top-level path, which
   ran none of the 17. Register D8c has the sha and the on-node provenance record. The
   quote is left verbatim because it is a quote, and the ladder is not this file's to edit.]
...
# sim_standing.py:132-133, the gated floor, IN THE DRIVER THAT PRODUCED THE 17:
# sha256 5215c38bed607ef6..., 389 lines, preserved at _incoming/sim_standing.py and
# recorded on-node at _incoming/conv_2026-07-26_idev/00_provenance.txt:6.
# RE-CORRECTED 2026-08-13, same day, second pass. An earlier edit that day repointed
# this to :210-211 and called :132 "stale triangle-rasterisation arithmetic". That was
# WRONG: :210-211 is the floor in the 2026-08-08 revision which later overwrote the
# top-level path and ran none of the 17. Register D8c. Arm JSONs already stamped on
# Vista carry the :210-211 string; expect a seam. The VALUE 0.55 was right throughout.
...
def run_arm(base_rung, mu, n_grid, seed, args):
    ...
    res["arm_provenance"] = {
        ...
        "friction_source": "sim_standing.py:132-133 in the GATED DRIVER (sha256 "
                           "5215c38bed607ef6..., 389 lines, preserved at "
                           "renders/yaris_render_s1/_incoming/sim_standing.py), floor "
                           "restitution=0.05 friction=0.55. NOT :210-211: that is the "
                           "2026-08-08 revision which later overwrote the top-level "
                           "path and ran none of the 17. Register D8c. Arms stamped "
                           "before 2026-08-13 carry the :210-211 string; value "
                           "unchanged at 0.55.",
