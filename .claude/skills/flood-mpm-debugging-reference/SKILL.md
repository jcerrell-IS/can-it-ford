...
7. **Coupling force validation, currently zero, and this is now mandatory, not optional.** No test, example, or benchmark anywhere in the Genesis repo validates MPM-rigid coupling force against an analytical or experimental reference (Archimedes buoyancy, Stokes drag, published data). Before trusting ANY force number this pipeline produces: submerge a rigid box of known volume in MPM water, compare the steady-state vertical force to `ρgV`. **ENGINE TAG, added 2026-08-18.** The accessor this line used to name, `cfrc_coupling_vel`, is **GENESIS-ONLY** (register A3), and the sentence around it was untagged while the canonical pipeline is warpmpm, so it read as a warpmpm instruction. In **warpmpm** the equivalent read is `sdf_wrench` on the SDF-collider path (`core/solver.py:354`, paired with `reset_sdf_force` at `:348` because the engine never zeroes the accumulator), which is exactly how `simulation/r5_physics/sphere_heave.py` measures the sphere reaction. On the **free-rigid material-8 path used by the 17 canonical runs there is no force accessor at all**, so this validation cannot be run there as written: that path exchanges momentum by mass-weighted grid-velocity averaging, and its net force cannot be decomposed. Repeat for a towed body vs. a drag estimate. Decision threshold: >5-10% deviation means reduce `dt`/raise `substeps`/raise `grid_density` before trusting anything else from the run.
