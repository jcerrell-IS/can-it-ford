...
- **LS6 IS x86_64, NOT aarch64.** Vista is the GH200 aarch64 machine; LS6 is the x86 one. The project's own Chrono build directories say so: the LS6 build is named `chrono_x86_build` while the aarch64 build lives on Vista's scratch. Routing an ARM workaround to LS6 wastes the session. Not confirmed by a live `uname -m` because LS6 needs MFA and the ControlMaster socket expires; run `bash scripts/tacc.sh ls6 'uname -m'` after an interactive `ssh ls6` to close that.
