...
## it's still a valid record of what was true that night)
...
CLAUDE.md's STILL OPEN / NOT YET CONFIRMED section for the current prioritized
list, this has now superseded the per-pane breakdown that used to live in a
separate audit file.

New finding not in the original document above: viability_audit.py globs
...
Current canonical status for the crash and all fixes now lives in CLAUDE.md
(Vista: /work/11603/jcerrell0629/vista/CLAUDE.md). Treat this document as
historical record of that night's plan, not as a live status source, from
this point forward.
