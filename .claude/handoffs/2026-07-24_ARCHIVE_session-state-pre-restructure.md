...
### 2026-07-24 02:58 UTC (2026-07-23 21:58 CDT), pane F5-SESSION-STATE commit-sourced verification (NEWEST; supersedes the 11:30 UTC header entry, extends the ~11:07 UTC block below, deletes neither)

**Method, so this block can be audited like any other.** Every hash below was re-read live this pass with `git show` / `git log` / `git cat-file` in `/Users/josie/can-it-ford` at HEAD `daf453e`. Every file claim was re-read live with `grep`/`cat` on the actual file, not from a summary. Vista claims were re-read live over ssh at ~02:5x UTC. Working tree at write time: clean except untracked `HANDOFF_AUDIT_2026-07-24/`. No history operation, force-push, or delete was performed or is being requested by this pane.

**P2G crash: re-confirmed NOT fixed by the position change (`d869f93`, 2026-07-23 10:52:51 CDT).** Diff verified live: the commit adds 8 lines to `SESSION_STATE.md` and touches no code. What it records: `8cc302c` moved the water box to `pos=-1.8` at 00:10:18; a crash trace at 00:27:22, 17 minutes later, showed an identical `CUDA_ERROR_ILLEGAL_ADDRESS` in p2g. Position was not the coupled variable causing this. Domain-widening bounds remain the live suspect per the 19:53 CDT entry. Re-confirmed independently this pass: `squeue -u jcerrell0629` on Vista returns zero jobs, and no new successful sim output exists. Do NOT report the coupled MPM sim as running or as fixed.

**Yaris mass: resolved to Option A, 1100 kg, and the live code agrees (`aa13ac1`, 2026-07-23 10:52:47 CDT).** The commit itself touches only `reference_data/vehicle_data_master_reference_2026-07-21.json` (+15/-10), no code. It adds `recommended_pairing_CORRECTED_2026_07_23`: Option A, 1100 kg at collider box volume 3.5427 m^3, rho 310.4976 (stored as 310.50), explicitly superseding the old Option B field (1078 kg, rho 304.28), which is kept for history rather than deleted. Stated basis: two independent live reads of `yaris-coarse-v1l.key` confirm 1100 kg is the ONLY mass value the deck header itself states; neither 1078 nor MASH appears anywhere in the deck, both being secondary NCAC-webpage annotations rather than primary source content.
- LIVE CODE CHECK, required before citing this and done this pass: `vehicle_params.py` line 83 reads `"mass_kg": 1100.0`, with line 82 commenting `curb weight, FE deck header ("Version 1l, 1100 kg")` and `mass_source` = `SRC["ncac_yaris_fe"]` (doi 10.13021/G8JS5D). `git status --porcelain vehicle_params.py` is empty, so this is committed state, not a pane's uncommitted edit. This upgrades the ~11:07 block's "Yaris mass, RECOMMENDATION only, not a fact until confirmed in code" to VERIFIED **for mass**.
- STILL OPEN, do not close: the volume basis. `aa13ac1` retains `volume_basis_still_open` (collider box 3.5427 m^3 vs raw enclosed mesh 6.8185 m^3, giving rho 310.50 vs 161.33). Mass is settled. **rho is NOT settled** until the volume basis is chosen. Do not paste any single rho into a script on the strength of the mass being fixed.
- PHANTOM HASH, new this pass: that same JSON text asserts "vehicle_params.py and the L2 sim already use 1100.0/310.47 as of commit `6d14dcb`". `git cat-file -t 6d14dcb` returns "Not a valid object name" in this clone. The 1100.0 half of the sentence is independently true (verified above), but **do not cite `6d14dcb`**. Same failure class as the `f623cd2` phantom already corrected in the 11:07 block. Note also the number drift inside one file: 310.47 in that sentence vs 310.50 stored in the field it points at.

**MCP connector state recorded across Mac, Vista, LS6 (`ee82867`, 2026-07-23 12:50:25 CDT).** Creates `token_setup_template.md` as a new 19-line file. Records all five project connectors (deepwiki, github, wandb, hf, scite) as Connected on all three machines, verified against the real services rather than just `claude mcp list` status. Root causes it preserves for the next rotation: an exposed GitHub PAT from a July 19 chat leak was still live and unrevoked until that session; GitHub's Copilot MCP endpoint needs the "Copilot Requests" account permission, not just repo scopes; `/tmp` on TACC nodes is not shared across idev allocations, use `~/.secrets_tmp`; a stale duplicate `huggingface` entry (wrong URL, `?login` suffix) alongside the working `hf` entry was removed. FLAG for Josie, not acted on: that tracked file records a truncated PAT prefix (`github_pat_11CDJE...`). Truncated, so not usable as a credential, but it is a token fingerprint committed to the repo.

**Pane tooling fixes, and the Stop hook mechanism verified live (`6b58811`, `0042612`).** `6b58811` (18:23:58 CDT) touches `scripts/resume_pane.sh` only, +28/-3, requiring an explicit fallback environment instead of a hardcoded Vista default. `0042612` (18:35:21 CDT) touches `.claude/settings.json` only, +2/-2.
- LIVE CHECK, required before citing and done this pass: `.claude/settings.json` right now defines the Stop hook as `mkdir -p ~/.pane_signals && echo "$(date -u +%s)" > ~/.pane_signals/$(tmux display-message -p '#S_#I_#P')_done`. That is the **file-based `~/.pane_signals/` mechanism, NOT tmux wait-for**. Confirmed present and current.
- Residue worth knowing: `"Bash(tmux wait-for*)"` is still listed in `permissions.allow` in that same file. It is a leftover permission grant, harmless in itself, but it is NOT the signaling mechanism and no pane should read it as one.
- OPERATIONAL CAVEAT found this pass, matters to anyone reading signals: the hook writes a unix timestamp and never clears old signal files. `~/.pane_signals/` currently holds only `ford_0_3_done` (20:45 CDT) and `ford_0_5_done` (21:00 CDT), both written BEFORE this round's task was issued. They are STALE. Compare each signal's mtime against round start; do not treat mere existence of a `*_done` file as this round's completion.

**Repo cleanup, nested-repo removal (`cdc6037`, `daf453e`).** `cdc6037` (18:35:46 CDT) reorganizes skills (adds `.claude/skills/claude-code-prompt-install/SKILL.md` and `.claude/skills/panel-audit-dispatch/SKILL.md`, retires the flat `.claude/skills/provenance-audit/provenance-audit.md`, 217 lines, in favor of the `SKILL.md` layout) and in the same commit accidentally added a gitlink entry `can-it-ford`, an embedded git repository. `daf453e` (19:01:44 CDT) removes that gitlink and adds it to `.gitignore`. Verified live: `git ls-files can-it-ford` returns 0 tracked files, so the tracking is genuinely gone.
- The DIRECTORY still exists on disk at `/Users/josie/can-it-ford/can-it-ford/` with its own working copies (for example `can-it-ford/vehicle_params.py`, `can-it-ford/kumar_july9_update/vehicle_params.py`). Untracked and ignored, not deleted. No pane should edit anything under that path believing it is live. Removing it is a delete and needs Josie's explicit confirmation; this pane did not act on it.

#### New findings this pass, not previously recorded in any file

**NEW 1. SECURITY: a live `CLAUDE_CODE_OAUTH_TOKEN` sits in plaintext in Vista's `~/.bashrc`.** Verified live over ssh this pass, with the value deliberately never read, printed, or copied anywhere. Evidence: `grep -n -o CLAUDE_CODE_OAUTH_TOKEN ~/.bashrc` returns `112:CLAUDE_CODE_OAUTH_TOKEN`, a single assignment at approximately line 112. The assigned value measures 103 characters, consistent with a populated live token rather than an empty placeholder. File permissions are `-rwx------` (owner only), mtime Jul 23 18:11, path `/home1/11603/jcerrell0629/.bashrc`. No other secret-like variable name was found in that file.
- Owner-only perms limit but do not remove exposure: it is still plaintext on a shared TACC filesystem, in scope for backups and snapshots, and readable by anything running as this account.
- STATUS: **UNROTATED and UNCONFIRMED**, same severity class as the prior WANDB key exposure, whose rotation was CLAIMED but never confirmed by actually comparing the live key against the exposed one.
- RECOMMENDATION, NOT executed by this pane: move it out of `~/.bashrc` into a gitignored `.env`-style file (for example `~/.secrets_tmp` or `~/.config/canitford/env`, `chmod 600`) sourced from `.bashrc`, the same pattern already used for the WANDB key; then rotate the token and confirm the rotation by comparing old against new, not by restating "rotated". Do NOT paste the value into any file, chat, log, or commit.
- Related and unscanned: `/work/11603/jcerrell0629/vista/can-it-ford-claude-sessions-2026-07-23.tar.gz` (10,272,055 bytes, Jul 23 06:06) is an archive of Claude session logs on the same filesystem. Session logs are a plausible carrier for the same secrets and this archive has not been scanned.

**NEW 2. A previously-unknown duplicate clone exists on Vista. Recorded only, not touched.** Verified live over ssh: `/work/11603/jcerrell0629/vista/can-it-ford-OLD-pre-purge/` exists, directory mtime Jul 23 08:23, parallel in purpose to the Mac's own `can-it-ford-BACKUP-before-history-purge`. It is a real distinct clone, not a symlink or a stale name: its HEAD is `0736dc3` "Add project identity/deadlines to CLAUDE.md, add repo-root STATUS.md, supersede old STATUS", with `dafc160` and `85f06f9` beneath it, none of which are on the live line.
- The LIVE Vista clone is `/work/11603/jcerrell0629/vista/can-it-ford` (directory mtime Jul 23 21:57), HEAD `daf453e`, which matches the Mac HEAD exactly. Mac and Vista are therefore in sync at `daf453e`.
- Why this is recorded: the OLD clone carries PRE-PURGE history, so it is exactly the artifact that could re-introduce purged content if anyone pushed or rewrote from it. Do not push, pull, or rewrite from that directory, and do not mistake it for the live clone. Any decision to delete it is Josie's and requires explicit confirmation. No pane should act on it.

**NEW 3. `designsafe-staging/` is NOT an external clone. The prior assumption was wrong, and the directory carries a stale rho.** Verified live: `git ls-files designsafe-staging/` returns 10 tracked files and `designsafe-staging/.git` does not exist, so it is an ordinary **tracked subdirectory of this same repo**, not a separate or external clone. Correct that assumption wherever it is repeated. Last touched by `2396dd5` "Strip inline comment from designsafe-staging script per no-comments rule" (2026-07-22 21:11:48 CDT, 1 file, +1/-1), preceded by `f364e7f` (coup_friction to 0.55, citing Azhar et al. 2023) and `ee4fa62`. Contents are a DesignSafe **PRJ-6388** submission-staging package: `MANIFEST.txt`, `docs/README_designsafe.md`, five figures, and three scripts.
- STALE PARAMETER, and it is NOT a copy of the live value. As first read this pass, `designsafe-staging/scripts/can_it_ford_L2.py` hardcoded `rho=604` twice, at line 40 (`gs.materials.Rigid(...)`) and line 131 (the npz record). Pane C2 fixed it in `606de5b` (2026-07-23 22:00:11 CDT) to **`rho=579.06`**, which landed between this pane's read and this pane's commit.
- **SELF-CORRECTION, this block's first draft was wrong on this point.** The first draft compared 604 against the live MPM's `VEHICLE_RHO = 115.7` and implied 115.7 was the target. That is the pattern-analogy error CLAUDE.md explicitly forbids: rho only means a mass against a specific volume, and these two scripts have independent box geometry. **Do NOT copy 115.7 into the staging script.**
- Derivation re-verified INDEPENDENTLY by this pane, both geometries read live, not taken from C2's message: live MPM `simulation/can_it_ford_L2_mpm.py:26-27` has `VEHICLE_SIZE = (4.66, 1.79, 1.44)`, V = 12.011616 m^3, so m = 115.7 * 12.011616 = 1389.744 kg. Staging script line 57 has `gs.morphs.Box(size=(1.0, 1.6, 1.5))`, V = 2.4 m^3, so rho = 1389.744 / 2.4 = **579.06 kg/m^3**. Units check: (kg/m^3) * m^3 = kg. Anchor check: 1389.744 kg is inside the 1000-1600 kg sedan band; 579.06 is below water at 1000 kg/m^3, so the body still floats. The old 604 implied 1449.600 kg, which is +59.86 kg (+4.31%) over the sedan target. C2's arithmetic is correct in full.
- CAVEAT C2 flagged and did not paper over, carried forward here: 579.06 is ABOVE the 100-300 kg/m^3 vehicle effective-density band in CLAUDE.md. That is forced by the frozen pilot geometry (a 2.4 m^3 block standing in for a 12.01 m^3 sedan volume), so matching mass necessarily breaks effective density. Consequence: **mass is the aligned quantity, draft is not.** Submerged fraction goes from 0.116 (draft 0.167 m) in the live MPM box to 0.579 (draft 0.869 m) here. Do not compare this script's output to the live MPM run on flotation depth.
- SOLVER, verified live and it matters for labeling: this staging script is `gs.materials.SPH.Liquid` (line 45) with `SPHOptions` (line 27). It is the frozen **SPH pilot**, not the live MPM scene. Anything shipped from it carries the SPH-PILOT caveat label, never MPM-REAL.
- ONE C2 CLAIM DOES NOT HOLD, checked against the artifact rather than the report: C2's commit message states "the separate, still-open run_tag bug in this file: run_tag reads `cf0p4` while the code sets coup_friction=0.55". Live grep of line 113 reads `..._mu0p005_cf0p55_...`, and the same is true at C2's own tip `606de5b`. `git log -S'cf0p4_'` shows that string was already removed by `f364e7f` ("Fix coup_friction to 0.55 in designsafe staging script, cite Azhar et al. 2023"). So the code bug is **already fixed, not still open**. The surviving valid concern is narrower: output files generated BEFORE `f364e7f` carry `cf0p4` in their filenames and do mislabel friction. Re-scope it to those artifacts, do not re-open the code bug.
- OWNERSHIP: `designsafe-staging/scripts/can_it_ford_L2.py` is pane C2's; no other pane should edit it.
- CONSEQUENCE, do not lose this: anything already submitted to or staged for DesignSafe PRJ-6388 out of this directory was generated at `rho=604` (1449.600 kg, 4.31% over sedan target), which is a different buoyancy regime from the corrected 579.06. Every figure and result in that package is therefore **UNVERIFIED until regenerated** against the corrected value. Nothing was pulled, edited, or resubmitted by this pane.

#### What this block deliberately does NOT claim
- It does not claim the MPM sim runs, renders, or produces a verdict. It claims the opposite, on a live `squeue` and on `d869f93`.
- It does not claim rho is settled. Mass is; the volume basis is not.
- It does not claim the Vista token is rotated, or that the earlier WANDB exposure is resolved. Both are UNCONFIRMED.
- It does not claim the DesignSafe package is correct. It flags it UNVERIFIED.
- It does not resolve the personal-profile leak recorded in the 11:07 block. That remains as written there.

### 2026-07-23 ~11:07 UTC full-round reconciliation (extends and corrects the 05:13 items below; itself extended by the 02:58 UTC block above)
